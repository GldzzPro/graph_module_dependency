...
export class MainModuleGraph extends Component {
   ...
   */
  async onClickModule(module) {
    const moduleId = module.id;
    const moduleNode = this.state.modules.find((n) => n.id == moduleId);
    ...
    console.log({ graphNodes: this.graphNodes._data });
    ...
    try {
      ...
      console.log({ method, moduleIds, options });
      ...
      console.log({
        ...
        graphEdges: this.graphEdges._data,
      });
    } catch (error) {
  ...
  onRemoveModule(moduleId) {
    this.state.selectedModules = this.state.selectedModules.filter(
      (m) => m.id !== moduleId
    );
    this.fetchGraph();
  }
  ...
  async fetchGraph() {
    ...
    const data = await this.orm.call("ir.module.module", method, [moduleIds], {
      options,
    });
